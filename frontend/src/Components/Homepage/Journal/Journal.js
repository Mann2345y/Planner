...
const Journal = () => {
  ...
  useEffect(() => {
    if (Object.keys(plans).length > 0) {
      const journalArray = plans.filter(
        (item) => item.typeOfData === "journal"
      );
    ...
    <AnimatePresence>
      {openEdit ? (
        <Motion
          ...
          exit={{ opacity: 0 }}
        >
          <Create setOpenEdit={setOpenEdit} currjournal={currjournal} />
        </Motion>
      ) : (
        <Motion
          key="noeditpage"
          ...
          <Wrapper>
            ...
            <TabsBlock>
              {status === "loading" ? (
                ...
                <NotFoundWrapper>
                  <NotFound>
                    No Journals Created, Click on Create button above to create
                    one.
                  </NotFound>
                </NotFoundWrapper>
              )}
